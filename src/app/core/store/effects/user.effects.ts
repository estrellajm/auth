...
export class UserEffects {
        ...
        switchMap(user => {
            if (user.additionalUserInfo.isNewUser) {
                ...
                return this.afs.doc<User>(`users/${user.uid}`).snapshotChanges().pipe(
                    ...
                    switchMap(() => of(new fromRouter.Go({ path: ['/user'] }))),
    ...
    logoutSuccess$: Observable<Action> = this.actions$.ofType(userActions.LOGOUT_SUCCESS).pipe(
    ...
    facebook_login$: Observable<Action> = this.actions$.ofType(userActions.FACEBOOK_LOGIN).pipe(
        ...
        map(() => { return new fromRouter.Go({ path: ['/dashboard'] }) }),
        catchError(err => {
            return Observable.of(new userActions.AuthError({ error: err.message }))
        })
    );
    ...
    load_user_shifts$: Observable<Action> = this.actions$.ofType(userActions.LOAD_USER_SHIFTS).pipe(
        ...
        switchMap(user => {
            if (user) {
                return this.afs.collection<User>(`users/${user.uid}/shifts`).stateChanges().pipe(
    ...
    @Effect()
    send_email_verification$: Observable<Action> = this.actions$.ofType(userActions.SEND_EMAIL_VERIFICATION).pipe(
    ...
    /// Forgot Password
    @Effect()
    forgot_password$: Observable<Action> = this.actions$.ofType(userActions.FORGOT_PASSWORD).pipe(
    ...
    @Effect()
    update_password$: Observable<Action> = this.actions$.ofType(userActions.UPDATE_PASSWORD).pipe(
    ...
    private getUserData(credentials) {
        ...
        let user = { ...credentials.user, ...credentials.additionalUserInfo.profile, ...credentials.additionalUserInfo.providerId }
