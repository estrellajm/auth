...
export const getUserID = createSelector(getUser, a => a['uid']);
export const getUserLoaded = createSelector(getUser, a => a['loaded']);
export const getUserLoading = createSelector(getUser, a => a['loading']);

export const getUserShifts = createSelector(getUser, a => {
  // console.log(a)
  const shifts = a['shifts'];
  return Object.keys(shifts).map(id => shifts[id]);
});
export const getUserShiftsPositions = createSelector(getUserShifts, a => {
  // console.log(a)
